do_lrm_pip <- function(data=subset_df(data_ml, "40w"), # data for model training
  ...
  infer_obj <- lrm_infer(df=df, df_org=data_org, dict_df=dict_df, fml=model_obj$cv_obj$model_info$formula, cluster_col=model_obj$cv_obj$model_info$cluster_col, num_col2=num_col2)
