dca <- function(data, outcome, predictors, xstart=0.01, xstop=0.99, xby=0.01, 
  ...
  interventionper=100, smooth=FALSE,loess.span=0.10, main="DCA") {
  ...
  if (graph==TRUE) {
    ...
    # PLOTTING NET BENEFIT IF REQUESTED
      ...
      plot(x=nb$threshold, y=nb$all, type="l", col=8, lwd=2 ,xlim=c(xstart, xstop), ylim=c(ymin, ymax), xlab="Threshold probability", ylab="Net benefit", main=main)
      # adding treat none option
      lines(x=nb$threshold, y=nb$none,lwd=2)
      #PLOTTING net benefit FOR EACH PREDICTOR
      for(m in 1:pred.n) {
        if (smooth==TRUE){
          lines(nb$threshold,data.matrix(nb[paste(predictors[m],"_sm",sep="")]),col=m,lty=2,lwd=2) 
        } else {
          lines(nb$threshold,data.matrix(nb[predictors[m]]),col=m,lty=2,lwd=2)
        }
        # adding each model to the legend
        legendlabel <- c(legendlabel, predictors[m])
        legendcolor <- c(legendcolor, m)
