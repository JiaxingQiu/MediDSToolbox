lrm_test <- function(
  ...
  if (is.null(test_data)){
    ...
    tryCatch({
      ...
      res_df <- data.frame(
        data = "external test data (engineered)",
        logloss = logloss,
        AUROC = AUROC,
        AUPRC = AUPRC,
        accuracy = accuracy,
    ...
    # ---- add predicted probability to orginal un-engineered test data ----
    tryCatch({
      if (is.null(test_data_org)) test_data_org <- test_data
      test_data_org$y_true <- as.numeric(as.character( test_data_org[,y_col] ) )
      test_data_org$y_prob <- as.numeric(logit2prob(predictrms(mdl_obj, newdata = test_data_org)) )
      test_data_org_score <- test_data_org %>% 
        filter(!is.na(y_prob) & !is.na(y_true) & y_true%in%c(0,1)) %>%
        as.data.frame()
      y_prob <- as.numeric(test_data_org_score$y_prob)
      y_pred <- as.numeric(ifelse(y_prob<=threshold,0,1))
      y_true <- as.numeric(test_data_org_score$y_true)
      stopifnot(all(!is.na(y_true)))
      stopifnot(length(y_pred)==length(y_true))
      stopifnot(length(y_prob)==length(y_true))
      logloss <- NA
      ...
      res_df <- bind_rows(res_df,
                          data.frame(
                            data = "external test data (original)",
                            logloss = logloss,
                            AUROC = AUROC,
                            AUPRC = AUPRC,
                            accuracy = accuracy,
      ...
      print("---- fail to get score df for test_data_org ----")
      print(e)
    })
  }
  
  test_data <- test_data[,union(c("y_prob",y_col),colnames(test_data))]
  test_data_org <- test_data_org[,union(c("y_prob",y_col),colnames(test_data_org))]
